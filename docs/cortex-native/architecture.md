...
| `native/src/scene/scene_mirror.*` | **Espelho da cena em C++** (SPEC-0233/0234): a hierarquia em memória LINEAR (pai antes de filho), composição da matriz de mundo e culling por frustum — as duas fases que em JS custavam 8,5 ms/frame. O JS descreve a cena UMA vez (`build`) e por frame só escreve transforms + `visible` num buffer compartilhado; as matrizes de mundo vivem aqui e o `three` as lê **sem cópia** (`matrixWorld.elements` aponta pra memória nativa). Guarda também, por nó, a autoria que o render precisa: `flags` ({@link NodeFlag}, 16 bits: castShadow autorado / isento do culling angular / frustumCulled / desenhável, mais os motivos de recusa do gate — skinned, instanced, material em array, recorte alfa, positionNode), `geometryId` (vínculo com o `GeometryRegistry`) e a esfera LOCAL da geometria. O que o `three` REAVALIA por frame (`object.visible`, `material.visible` e o **lado da face do passe de sombra** — `ShadowSide`, dois bits) não fica nas flags: viaja num campo de bits do buffer de sincronização (`SyncFlag`), porque uma foto do `build` envelhece em silêncio. **A cena CRESCE, mas por capacidade reservada** (SPEC-0245, E6): o `build` faz `reserve` com folga (`kMirrorSpareNodes`) em todos os vetores ANTES do `resize`, e `appendBatch` põe uma subárvore nova sem realocar — os `subarray` já entregues ao JS seguem válidos e só o nó novo é apontado. `removeSubtree` tira o nó e os descendentes virando **lápide no lugar** (índice de ninguém muda; o slot volta a ser usado por um append futuro, com a regra de que o slot tem de vir DEPOIS do pai). ⚠️ **Estouro de capacidade recusa, nunca realoca**: realocar deixaria todo `matrixWorld.elements` já entregue sobre memória liberada, e erro nessa fronteira aparece como artefato visual, não como exceção. Quem descobre a mutação é o lado JS, por evento `childadded`/`childremoved` — ver `src/core/NativeSceneMirror.ts`. |
| `native/src/scene/shadow_caster_enumerator.*` | **Quem projeta sombra neste frame** (SPEC-0245, M6). Reproduz os filtros do `three` NA ORDEM dele: visibilidade herdada (pai invisível poda a subárvore), malha desenhável, `castShadow` AUTORADO, culling angular da SPEC-0197 e frustum da ortho da cascata. Não desenha nada e não toca em GPU — por isso roda no alvo PURO `cortex_host_tests`. Validado contra os draws reais do `three` (por uma sonda de `renderObject`, já removida): com a cena parada, 2.632/2.632 amostras idênticas. |
| `native/src/scene/shadow_pass_gate.*` | **Quando o passe nativo NÃO pode assumir** (SPEC-0245, E3 do M6). Recebe o espelho, a lista de casters e os dois fatos que só o JS enxerga (nós da cena, tipo do shadow map) e devolve aceito/recusado com o motivo e a contagem por motivo. Recusar devolve o passe ao `three` e tudo segue como hoje — é o princípio do M1, **recusar em vez de aproximar**, e a assimetria justifica: aceitar por engano é sombra errada na imagem; recusar por engano é só não ganhar os milissegundos do marco. Motivos: divergência de nós cena×espelho, VSM, caster skinado, instanced, material em array, `alphaTest`/`alphaMap`, `positionNode`, geometria ausente do registro e **lado da face não reproduzível** (`side` fora da tabela do `three`, ou materiais do mesmo nó discordando). Também PURO — a consulta ao `GeometryRegistry` (que arrasta `webgpu.h`) entra por ponteiro de função, e por isso cada condição é exercitada no `cortex_host_tests`. |
| `native/src/render/shadow_pass.*` + `shadow_math.h` | **O passe de sombra depth-only em C++** (SPEC-0245, E5 do M6) — o caminho PADRÃO do host desde o E8. Desenha os casters enumerados direto na `ShadowDepthTexture` da cascata, no lugar do passe que o `three` faz em JS. Três decisões que não se adivinha: **sem attachment de cor** (nem estágio de fragmento — um fragmento que não escreve nada custaria uma invocação por pixel), **`cullMode` POR CASTER**, derivado do `side` do material (o `three` inverte o lado da face no passe de sombra: `_shadowSide` mapeia `FrontSide → BackSide`, `BackSide → FrontSide`, `DoubleSide → DoubleSide`; com `Back` fixo a profundidade de um caster `FrontSide` sairia da face errada e daria acne/peter-panning, e com `Front` fixo a folhagem `DoubleSide` perde a auto-sombra — premissa 4 da SPEC-0246). Os itens são agrupados por `(cull, passo de vértice)` e há um pipeline por par; `shadowCullMode` ainda reproduz o `flipSided` do `three` (escala espelhada, `determinant() < 0`, troca o sentido da face) e o alvo vem por **identidade do objeto** que o JS entrega, **reaquirido por frame** — nunca por dimensão, que é o que custou dois dias no M5. A matriz `viewProj × model` é composta em **double** (`shadow_math.h`, PURO e testado no harness) e só o resultado vira `float`: degradar antes devolve as bandas da SPEC-0234. Desliga com `?semPasseDeSombraNativo=1`. Diagnóstico: `CORTEX_SHADOW_PASS_LOG=1` imprime as primeiras chamadas com alvo, formato e casters. ⚠️ Com ele ativo, `shadow.autoUpdate = false` por cascata e o `three` para de desenhar a sombra — e de chamar `shadowMap.setSize`. Medido no kart-racer: −2,85 ms de `cpu.render` e 63 → 75 fps (remedido depois do `side`: −4,75 ms, 51 → 66 fps, numa máquina mais carregada). |
| `native/src/shims/scene_mirror_shim.*` | Ponte `__cortexSceneMirror`: `build` (cena inteira, 21 floats por nó), `worldMatrices`/`syncBuffer` (`ArrayBuffer` EXTERNO, sem cópia), `update` (a ÚNICA travessia de ponte por frame), `appendNodes`/`removeNode` (a cena que muda depois do `install`: a subárvore inteira numa chamada só, com o pai dos nós de dentro codificado por posição no lote) e `drawShadowPass` (enumera + gate + DESENHA numa travessia só; o `viewProj` atravessa em `Float64Array`, nunca `Float32Array`, e as contagens do gate voltam num `Float64Array` que o JS já tem, em vez de alocar um objeto por frame). |
...
| `native/scripts/build-host.ps1` | Compila o `rapier_native.dll` (cargo, SPEC-0260) e depois configura e compila o host (SPEC-0247). Exposto por `yarn build:host`. Antes o cargo era passo manual: esquecê-lo linkava o host contra a DLL velha SEM erro, e a função nova só faltava no export rodando. Trata os quatro cuidados que falham **sem acusar erro**: `vcvars64` e `cmake` na MESMA invocação do `cmd`, clang-cl por caminho completo (senão o CMake pega o `cl.exe` sem avisar), `native/build` nunca junction (o cache do CMake passaria a compilar outra worktree), e `--Limpar` para quando fontes saem do `CMakeLists.txt`. **Sequência de rebuild completo:** `fetch-deps.ps1` (só se as deps mudaram) → `yarn build:host` → `yarn build:engine` (sempre completo: build parcial apaga o `rapier.js` por `emptyOutDir`) → `yarn electron:build`. |
...
| `native/scripts/export-game.mjs` | Export distribuível (ADR-0101): bundle+hermesc -O+exe+dlls+assets.pak → `<jogo>/dist-native/`. O exe é **fixo `launcher.exe`** e o nome/id do jogo vêm do `cortex.json` (ADR-0126, `game-config.mjs`) — grava um `cortex.json` **resolvido** (id/name garantidos) no dist. Também copia soltos (fora do pak): `config.ini` e a pasta `languages/` INTEIRA (i18n editável sem rebuild, SPEC-0124). |
| `native/scripts/copy-languages.mjs` | `copyLanguages(gameDir, dist)` — copia `languages/` **recursivamente** pro dist, sem filtro de extensão (ADR-0187). Além dos `.txt`, leva os assets localizados das subpastas: placas dos portais (`signs/<lang>/*.png`) e dublagem por idioma (`voice/<lang>/`). O host acha esses arquivos porque o `__cortexReadFile` cai pro disco quando a chave não está no `assets.pak`. |
...
| `native/export-toolchain/` | Toolchain de export AUTO-CONTIDO (TDR-0003): `package.json`+`yarn.lock` pinados com **todo import bare do grafo do export** — esbuild/babel/three/three-mesh-bvh/zod (bundle), `@gltf-transform/core`+`extensions` (cook) e png-to-ico/rcedit (ícone). O CI instala e o electron-builder copia o `node_modules` pra `resources/node_modules` (só Windows), pro Studio empacotado exportar sem dev. **Armadilha (SPEC-0177):** dep que a engine tem como devDependency resolve em dev e some no `.exe`; e ela nem sempre aparece como `import` (o Babel resolve plugin por NOME). Dois testes cobrem: `export-packaging.test.ts` (grafo estático) e `export-isolated-smoke.test.ts` (roda o `bundle.mjs` REAL com o toolchain como única árvore de módulos — pega qualquer forma de dep faltando). |
...
| `native/scripts/ensure-host-dirs.mjs` | Roda antes do electron-builder (`electron:build`): garante os `from` de `win.extraResources` que podem faltar — `native/build-steam` vazio quando não há SDK (ADR-0176) e o **encoder KTX2** baixado/criado em `native/tools/basis-encoder` (SPEC-0177). Sem isso o electron-builder aborta com "source doesn't exist". |
...
- **O caminho de render nativo depende do COMPORTAMENTO INTERNO do `three`, e
  ...
  do lado da face (`_shadowSide`, a TABELA inteira, que decide o `cullMode` de
  cada caster), o filtro
