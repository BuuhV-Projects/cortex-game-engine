...
| `native/export-toolchain/` | Toolchain de export AUTO-CONTIDO (TDR-0003): `package.json`+`yarn.lock` pinados com **todo import bare do grafo do export** — esbuild/babel/three/three-mesh-bvh/zod (bundle), `@gltf-transform/core`+`extensions` (cook) e png-to-ico/rcedit (ícone). O CI instala e o electron-builder copia o `node_modules` pra `resources/node_modules` (só Windows), pro Studio empacotado exportar sem dev. **Armadilha (SPEC-0177):** dep que a engine tem como devDependency resolve em dev e some no `.exe` — `tests/native/export-packaging.test.ts` caminha o grafo e cobra o pin no CI. |
