...
| `native/src/shims/scene_mirror_shim.*` | Ponte `__cortexSceneMirror`: `build` (cena inteira, 20 floats por nó), `worldMatrices`/`syncBuffer` (`ArrayBuffer` EXTERNO, sem cópia), `update` (a ÚNICA travessia de ponte por frame) e `shadowCasters` (contagem do enumerador, para conferência). |
