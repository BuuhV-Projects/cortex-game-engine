...
| `native/src/shims/scene_mirror_shim.*` | Ponte `__cortexSceneMirror`: `build` (cena inteira, 20 floats por nó), `worldMatrices`/`syncBuffer` (`ArrayBuffer` EXTERNO, sem cópia), `update` (a ÚNICA travessia de ponte por frame) `shadowCasters` (contagem do enumerador, para conferência) e `shadowPassGate` (veredito do gate; escreve as contagens num `Float64Array` que o JS já tem, em vez de alocar um objeto por frame). |
