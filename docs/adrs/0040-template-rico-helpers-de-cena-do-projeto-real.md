# 0040 - Template rico e helpers de cena vindos de um projeto real
...
- Um `placeOnGround` que, além de assentar a base, **centra horizontalmente** e
  ...
  `placeOnGround(obj, groundY)` de só-Y do ADR-0039.
...
1. **`SceneAssets.ts`** (substitui o `Placement.ts` do ADR-0039): `loadGLB`,
...
- `placeOnGround` mudou de assinatura (era novíssimo, do ADR-0039, e não chegou a
  ser publicado/usado fora deste repo) — ADR-0039 marcado como refinado por este.
...
- Relaciona-se com ADR-0037 (inspect_assets), 0038 (editor no template) e 0039.
