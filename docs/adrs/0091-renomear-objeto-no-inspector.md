# 0091 - Renomear objeto no Inspector (só nós adicionados no editor)
...
objeto pelo nome) e com o drag-and-drop (ADR-0090), que gera ids automáticos
...
- **`RenameAuthoring`** (`src/editor/authoring/`, padrão ADR-0060):
  ...
  - **Undo**: o `attachEditor` registra o comando no CommandStack (ADR-0084) via
