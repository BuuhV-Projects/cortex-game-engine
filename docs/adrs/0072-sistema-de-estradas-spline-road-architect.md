# 0072 - Sistema de estradas por spline (inspirado no Road Architect)
...
texturas** portam bem — exatamente como o ProBuilder (ADR-0071) trouxe blockout pro modelo
...
autora a spline (adicionar/mover/inserir nós), overlay vence (igual ao `mesh`/ADR-0071).
...
- **`src/road/` novo** (spline + mesh) — puro, testável, reusável (a IA do Chat pode gerar
  nós `road`). Segue o molde do `src/probuilder/` (ADR-0071).
