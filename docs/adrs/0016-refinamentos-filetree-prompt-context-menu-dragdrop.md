# 0016 - Refinamentos do FileTree: prompt custom, context menu, drag & drop
...
Após implementar criar arquivo (ADR-0011) e criar pasta (ADR-0015), três
...
- `fs:move` e `fs:delete` operam onde o usuário pediu — sem restrição a
  pasta específica (segue a mesma lógica da ADR-0013 sobre `fs:writeFile`).
