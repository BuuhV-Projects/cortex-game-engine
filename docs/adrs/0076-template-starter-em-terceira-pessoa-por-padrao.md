...
O SPEC-0064 definiu o demo padrão do template (`templates/new-project/`) como **1ª
pessoa**: `main.ts` com `setupFirstPerson` e `level.json` com um player `primitive`
cilindro. Depois, o SPEC-0074 portou o **controle de 3ª pessoa** (`setupThirdPerson`,
...
open-world/aventura (o caso de uso que motivou o port no SPEC-0074) e mostra mais do
...
2. `templates/new-project/scenes/level.json` troca o player `primitive` cilindro por
   ...
   `buildScene`).
3. A 1ª pessoa **continua disponível** como `setupFirstPerson` (SPEC-0064) — só deixou
   de ser o default do starter.

## Consequências

- O primeiro contato com o engine é um jogo 3D em **3ª pessoa com personagem
  animado** — exercita modelo rigado + animação (SPEC-0054) + câmera orbital
  (SPEC-0074) sem código extra. Coerente com "3D por padrão" (ADR-0062).
- **Supersede o item 3 do SPEC-0064** (demo padrão = 1ª pessoa / cilindro). O
