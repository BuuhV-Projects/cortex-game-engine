[**cortex-game-engine**](../README.md)

***

[cortex-game-engine](../README.md) / buildShape

# Function: buildShape()

> **buildShape**(`kind`, `params?`): [`EditableMesh`](../interfaces/EditableMesh.md)

Defined in: [.claude/worktrees/perf-boot-nativo/src/probuilder/shapes.ts:402](https://github.com/BuuhV-Projects/cortex-game-engine/blob/main/src/probuilder/shapes.ts#L402)

Gera a [EditableMesh](../interfaces/EditableMesh.md) de uma forma. Params ausentes caem no default.

## Parameters

### kind

[`ShapeKind`](../type-aliases/ShapeKind.md)

### params?

`Record`\<`string`, `number`\>

## Returns

[`EditableMesh`](../interfaces/EditableMesh.md)
