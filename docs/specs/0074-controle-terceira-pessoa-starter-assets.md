# SPEC-0074 - Controle de terceira pessoa (port do Unity StarterAssets)
...
O demo padrão de projeto novo era 1ª pessoa (`setupFirstPerson`, SPEC-0064). Faltava um
...
- **Animação** (idle/walk/run/jump/fall) dirigida pelo estado (velocidade horizontal +
  `grounded` + `velocityY`) via `deriveLocomotion`/`autoMapPlayerClips` (SPEC-0054) tocando
...
- **Reusa** CharacterBody/CharacterPhysics (gravidade/pulo/colisão de parede — SPEC-0071) e
  o sistema de animação data-driven (SPEC-0054). Sem física nova.
