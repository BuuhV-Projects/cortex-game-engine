...
// cascata, no lugar do passe que o `three` faz em JS. Três diferenças em
// relação ao `native_pass.cpp` do M5, e todas as três foram medidas antes de
// virar código:
//
...
//    entrega. Identificar alvo por dimensão foi o que custou dois dias no M5,
//    e por isso a heurística `cenaAlvo()` de `commands.cpp` não entra aqui.
//  - **`cullMode = Front`.** O `three` inverte o lado da face no passe de
//    sombra (`_shadowSide`: `FrontSide → BackSide`). Com `Back`, a
//    profundidade sairia da face errada — acne e peter-panning.
#pragma once
